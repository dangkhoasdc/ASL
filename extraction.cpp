...
cv::Mat extract(ConMatRef _input) {
    ...
    for (unsigned int i = 0; i < SIZEOF_ARR(theta); ++i) {
        for (unsigned int j = 0 ; j < SIZEOF_ARR(sigma); ++j) {
            cv::Mat result;
            cv::Mat kernel = cv::getGaborKernel(cv::Size(8,8),
                    sigma[j],
                    theta[i],
                    10.0,
                    0.5
    ...
    for (int k = 0; k < 16; k++) {
        ...
        for (int j = 0; j < WIDTH_CELL; j++) {
            ...
            for (int jj = 0; jj < response_size.width; jj++) {
                float xi = 16 * i - 8;
                float yj = 16 * j - 8;
                cell_value += responses[i].at<float>(ii,jj)
                    *exp(-(pow(ii - xi, 2.0) + pow(jj - yj,2.0))/(2* WIDTH_GABOR_FILTER * WIDTH_GABOR_FILTER));
            }
            }
